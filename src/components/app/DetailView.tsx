...
export function DetailView() {
  ...
  const isFav = useAppStore((s) =>
    placeId ? s.favorites.includes(placeId) : false
  );
  ...
  const related = places
    .filter(
      (p) =>
        p.id !== place.id &&
        (p.type === place.type || p.neighborhood === place.neighborhood)
    )
    ...
    <div className="min-h-screen pb-28">
            ...
            >
              <Heart
                className={`h-[18px] w-[18px] transition-colors ${
                  isFav ? "fill-accent text-accent" : "text-white"
                }`}
                strokeWidth={2.4}
              />
            </motion.span>
          ...
          >
            <div className="mb-2 flex flex-wrap gap-1.5">
              {place.tags.slice(0, 3).map((t) => (
                <span
                  key={t}
                  className="rounded-full bg-white/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white ring-1 ring-white/20 backdrop-blur-md"
                >
                  {t}
                </span>
              ))}
            </div>
            <h1 className="text-[30px] font-bold leading-[1.05] tracking-[-0.025em] text-white">
              {place.name}
            </h1>
      ...
      <div className="sheet-overlap rounded-t-[2rem] bg-background px-4 pb-2 pt-5">
        ...
        >
          ...
          <p className="mt-1.5 text-[18px] font-semibold leading-snug text-balance tracking-[-0.01em]">
            {place.vibeLine}
          </p>
        </motion.div>
        ...
        >
          ...
          >
            <p className="mt-1 text-[15px] font-bold tracking-tight">
              {TIME_LABEL[place.besteTijd] ?? place.besteTijd}
            </p>
            <p className="text-[11px] font-medium text-muted-foreground">
              {place.besteTijd === "Sunset"
                ? "Gouden uur"
                : place.besteTijd === "Morning"
                  ? "Vroege ochtend"
                  : "Avond / nacht"}
            </p>
          </InfoCard>
          ...
          >
            <div className="no-scrollbar flex gap-2.5 overflow-x-auto pb-1.5 -mx-4 px-4">
              {place.gallery.map((img, i) => (
                <div
                  key={i}
                  className="relative h-44 w-32 shrink-0 overflow-hidden rounded-2xl bg-muted shadow-premium ring-1 ring-black/5"
                >
                  <img
                    src={img}
                    alt={`${place.name} ${i + 1}`}
        ...
        >
          ...
          <ul className="mt-2.5 space-y-2">
            {place.tips.map((tip, i) => (
              <li
                key={i}
                className="flex gap-3 rounded-2xl bg-card p-3.5 text-[14px] leading-snug shadow-premium ring-1 ring-black/[0.03]"
              >
        ...
        >
          ...
          <div
            className="mt-2.5 flex items-center gap-3 rounded-2xl bg-card p-3.5 shadow-premium ring-1 ring-black/[0.03]"
            style={{
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] font-medium text-muted-foreground">
                {place.neighborhood} · {place.lat.toFixed(4)},{" "}
                {place.lng.toFixed(4)}
              </p>
            </div>
          </div>
        ...
        {related.length > 0 && (
          <motion.section
            ...
            className="mt-7"
          >
            <h3 className="mb-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
              Meer ontdekken
            </h3>
            <div className="no-scrollbar flex gap-2.5 overflow-x-auto pb-1.5 -mx-4 px-4">
              {related.map((p, i) => (
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-md glass-strong px-4 pt-3 pb-safe hairline-t">
        ...
        >
          <Navigation className="h-[18px] w-[18px]" strokeWidth={2.6} />
          Open in Google Maps
        </a>
      </div>
    </div>
