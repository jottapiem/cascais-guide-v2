...
export function DetailOverlay({ placeId, sectionId }: { placeId: string; sectionId: string }) {
  ...
  return createPortal(
    <>
      ...
      {/* ── STICKY MAPS BUTTON — the spec's Bottom Bar (phase 5) ──
          Three things it must do, all of them different from every other element here:
          it enters LAST and strictly AFTER the content crossfade has finished (not in
          parallel with it), it moves by translate only and never fades, and it is the one
          element with genuine bounce — an underdamped spring that visibly overshoots its
          resting position before settling back, where everything else in the morph is
          critically damped and never overshoots.

          Delay is the full crossfade duration, because the crossfade starts on the very
          frame this component flips to idle. Was 50ms, which put the bar's rise in the
          middle of the crossfade. Reverse/instant paths keep the plain duration: there is
          nothing to bounce out of on the way back, and the bar must be gone at once. ── */}
      <motion.div
        initial={{ y: "100%" }}
        animate={{ y: (exiting || morphPhase !== "idle") ? "100%" : 0 }}
        exit={{ y: "100%" }}
        transition={
          (exiting || morphPhase !== "idle")
            ? { duration: 0, delay: 0 }
            : morphPlace
              ? { ...BOTTOM_BAR_SPRING, delay: BOTTOM_BAR_DELAY_MS / 1000 }
              : { duration: BOTTOM_BAR_RISE_MS / 1000, delay: 0, ease: "easeOut" }
        }
