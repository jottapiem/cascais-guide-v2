...
// Mix a hex color with the background (#E8E6E3) at a given opacity (0-1)
...
function AirbnbCard({ place, index, userCoords }: { place: Place; index: number; userCoords: { lat: number; lng: number } | null }) {
  ...
  return (
    <BlurFade delay={Math.min(index * 0.04, 0.3)}>
      <motion.div
        onClick={() => { goDetail(place.id); haptics.selection(); }}
        whileTap={{ scale: 0.97 }}
        transition={{ duration: 0.15, ease: SWIFT_EASE }}
        className="w-64 shrink-0 cursor-pointer text-left"
      >
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl" style={{ boxShadow: CARD_SHADOW, border: "1px solid rgba(0,0,0,0.05)", background: "#F7F6F4" }}>
          <img src={place.coverImage} alt={place.name} loading="lazy" onLoad={() => setImgLoaded(true)} className={`h-full w-full object-cover transition-all duration-700 ease-out ${imgLoaded ? "opacity-100" : "opacity-0"}`} />
          {!imgLoaded && <div className="absolute inset-0 skeleton-shimmer" />}
          <button type="button" onClick={(e) => { e.stopPropagation(); toggleFavorite(place.id); haptics.light(); }} className="absolute right-2.5 top-2.5 flex h-8 w-8 items-center justify-center rounded-full bg-black/15 backdrop-blur-md transition-all active:scale-90" aria-label={isFav ? "Verwijder favoriet" : "Voeg toe aan favorieten"}>
            <Heart className={`h-4 w-4 transition-all ${isFav ? "fill-white text-white" : "text-white"}`} strokeWidth={2.5} />
          </button>
        </div>
        <div className="mt-1 px-0.5">
          <p className="truncate text-[14px] font-semibold text-neutral-700">{place.shortName}</p>
          <p className="mt-0.5 text-[13px] text-neutral-400">
            {place.neighborhood}
            {distance !== null && <span className="text-neutral-300"> · {distance} km</span>}
          </p>
          <div className="mt-0.5 flex items-center gap-1.5">
            <span className="text-[13px] text-neutral-400 capitalize">{place.type.toLowerCase()}</span>
            <span className="text-neutral-300">·</span>
            <Star className="h-3 w-3 fill-neutral-400 text-neutral-400" />
            <span className="text-[13px] font-normal text-neutral-400">{place.rating.toFixed(1)}</span>
          </div>
        </div>
      </motion.div>
    </BlurFade>
...
function SeeAllButton({ places: seePlaces, onClick }: { places: Place[]; onClick: () => void }) {
  ...
  return (
    <motion.button type="button" onClick={() => { onClick(); haptics.selection(); }} whileTap={{ scale: 0.96 }} transition={{ duration: 0.15, ease: SWIFT_EASE }} className="flex w-36 shrink-0 flex-col items-center justify-center gap-3">
      <div className="relative h-28 w-full">
        {top3.map((p, i) => (
          <div key={p.id} className="absolute h-20 w-16 overflow-hidden rounded-xl border-[3px] border-[#F7F6F4]" style={{ left: `${28 + i * 22}px`, top: `${8 + (i % 2 === 0 ? 0 : 6)}px`, transform: `rotate(${(i - 1) * 7}deg)`, zIndex: 3 - i, boxShadow: CARD_SHADOW }}>
            <img src={p.coverImage} alt={p.name} className="h-full w-full object-cover" />
          </div>
        ))}
      </div>
      <span className="flex items-center gap-1 rounded-full px-4 py-2 text-[12px] font-semibold text-neutral-700" style={{ boxShadow: CARD_SHADOW, border: "1px solid rgba(0,0,0,0.05)", background: "#F7F6F4" }}>Bekijk alles <ArrowRight className="h-3 w-3" /></span>
    </motion.button>
  );
}

function CategoryPill({ cat, isActive, onClick }: { cat: typeof CAT_BUBBLES[0]; isActive: boolean; onClick: () => void }) {
  // Active: subtle color tint in bg + colored icon
  // Inactive: neutral bg + gray icon
  ...
  return (
    <button data-cat={cat.id} type="button" onClick={() => { onClick(); haptics.selection(); }} className="flex shrink-0 items-center active:scale-95 transition-transform">
      <motion.div
        ...
        className="flex items-center gap-2.5 rounded-full px-5 py-2.5 transition-all duration-300"
        style={{
          background: activeBg,
          boxShadow: isActive ? PILL_HOLLOW : PILL_RAISED,
        }}
      >
        <Icon icon={cat.icon} className="h-5 w-5 transition-colors duration-300" style={{ color: iconColor }} strokeWidth={1} />
        <span className="text-[14px] font-semibold transition-colors duration-300" style={{ color: "#374151" }}>{cat.label}</span>
      </motion.div>
    </button>
  );
}
...
export function HomeView() {
  ...
  return (
    <div className="min-h-screen pb-4">
      <header
        className="sticky top-0 z-30 backdrop-blur-2xl transition-shadow duration-300"
        style={{
      ...
      >
        <div className="px-4 pt-safe-lg pb-3">
          <motion.button
            type="button"
            onClick={() => { goSearch(); haptics.selection(); }}
            whileTap={{ scale: 0.98 }}
            transition={{ duration: 0.15, ease: SWIFT_EASE }}
            className="flex w-full items-center justify-center rounded-full px-4 py-3 text-left"
            style={{ boxShadow: SEARCH_SHADOW, background: "#F7F6F4" }}
          >
            <Search className="h-[16px] w-[16px] text-neutral-500" strokeWidth={2.2} />
            <span className="ml-2 flex-1 text-center text-[14px] text-neutral-500">Begin met ontdekken</span>
          </motion.button>
        </div>
        <div ref={catScrollRef} className="no-scrollbar flex gap-2.5 overflow-x-auto px-4 py-2">
          {CAT_BUBBLES.map((cat) => (
            <CategoryPill key={cat.id} cat={cat} isActive={activeCat === cat.id} onClick={() => setActiveCat(cat.id)} />
          ))}
        </div>
      </header>
      ...
      {activeCat !== "all" ? (
        <div className="px-4 pt-3">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[18px] font-bold tracking-tight text-neutral-700">{CAT_BUBBLES.find((c) => c.id === activeCat)?.label}</h2>
            <button type="button" onClick={() => { goMap(); haptics.selection(); }} className="flex items-center gap-1 rounded-full px-3 py-1.5 text-[12px] font-semibold text-neutral-700" style={{ boxShadow: CARD_SHADOW, border: "1px solid rgba(0,0,0,0.05)", background: "#F7F6F4" }}><MapIcon className="h-3 w-3" /> Kaart</button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {filteredFeed.map((place, i) => (<AirbnbCard key={place.id} place={place} index={i} userCoords={userCoords} />))}
          </div>
        </div>
      ) : (
        <div className="pt-3">
          <Section title="Populair in Cascais" places={topRated} onSeeAll={goMap} userCoords={userCoords} />
          <Section title="Trending nu" places={trending} onSeeAll={() => goCategory("trending")} userCoords={userCoords} />
          <Section title="Hidden gems" places={hiddenGems} onSeeAll={() => goCategory("viewpoints")} userCoords={userCoords} />
          <Section title="Beste sunset spots" places={sunsetSpots} onSeeAll={() => goCategory("sunset")} userCoords={userCoords} />
          <p className="px-4 pb-4 pt-6 text-center text-[11px] text-neutral-400">{places.length} plekken · Cascais · Estoril · Guincho · Sintra</p>
        </div>
      )}
    </div>
  );
...
function Section({ title, places: sectionPlaces, onSeeAll, userCoords }: { title: string; places: Place[]; onSeeAll: () => void; userCoords: { lat: number; lng: number } | null }) {
    ...
    <section className="mb-6">
      <BlurFade>
        <div className="mb-3 flex items-center justify-between px-4">
          <h2 className="text-[18px] font-bold tracking-tight text-neutral-700 pb-2 border-b border-black/[0.04] w-full">{title}</h2>
        </div>
      </BlurFade>
      <div className="no-scrollbar flex gap-3 overflow-x-auto px-4 pb-1">
        {sectionPlaces.slice(0, 8).map((place, i) => (<AirbnbCard key={place.id} place={place} index={i} userCoords={userCoords} />))}
