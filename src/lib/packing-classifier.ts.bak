...
export function classifyItem(name: string): PackingCategory {
  ...
  for (const rule of RULES) {
    for (const kw of rule.kw) {
      if (n === norm(kw) || n.includes(norm(kw))) {
        return rule.cat;
      }
    }
  }
  // fuzzy fallback: closest keyword
...
export const SMART_COLLECTIONS: SmartCollection[] = [
  ...
  { id: "col-vliegtuig", situation: "Vliegtuig", icon: "Plane", items: ["Paspoort", "Boarding pass", "Telefoon", "Oplader", "Koptelefoon", "Paspoort", "Handdesinfectie"] },
